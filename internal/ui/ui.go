...
func (h *Handler) Register(mux *http.ServeMux) {
	...
	mux.Handle("/", CSRFMiddleware(!h.devMode, inner))
}
...
func (h *Handler) registerRoutes(mux *http.ServeMux) {
	mux.HandleFunc("GET /{$}", h.handleRepos)
	mux.HandleFunc("GET /u/{identity}", h.handleUserProfile)
	...
	mux.HandleFunc("GET /r/{owner}/{name}/b/{branch...}", h.handleBranchGetDispatch)
	mux.HandleFunc("GET /_/r/{owner}/{name}/b/{branch...}", h.handleBranchPartialGetDispatch)
	mux.HandleFunc("GET /r/{owner}/{name}/f/{path...}", h.handleFile)
	mux.HandleFunc("GET /r/{owner}/{name}/issues", h.handleIssues)
	mux.HandleFunc("GET /r/{owner}/{name}/issues/new", h.handleNewIssue)
	mux.HandleFunc("POST /r/{owner}/{name}/issues/new", h.handleNewIssue)
	mux.HandleFunc("GET /r/{owner}/{name}/issues/{number}", h.handleIssueDetail)
	mux.HandleFunc("POST /r/{owner}/{name}/issues/{number}/edit", h.handleEditIssue)
	mux.HandleFunc("POST /r/{owner}/{name}/issues/{number}/close", h.handleIssueClose)
	mux.HandleFunc("POST /r/{owner}/{name}/issues/{number}/reopen", h.handleIssueReopen)
	mux.HandleFunc("POST /r/{owner}/{name}/issues/{number}/comments", h.handleCreateIssueComment)
	mux.HandleFunc("POST /r/{owner}/{name}/issues/{number}/comments/{id}/edit", h.handleEditIssueComment)
	mux.HandleFunc("POST /r/{owner}/{name}/issues/{number}/comments/{id}/delete", h.handleDeleteIssueComment)
	mux.HandleFunc("POST /r/{owner}/{name}/issues/{number}/refs", h.handleAddIssueRef)
	mux.HandleFunc("GET /_/r/{owner}/{name}/issues", h.handleIssuesPartial)
	mux.HandleFunc("GET /r/{owner}/{name}/proposals", h.handleProposals)
	mux.HandleFunc("GET /_/r/{owner}/{name}/proposals", h.handleProposalsPartial)
	mux.HandleFunc("GET /r/{owner}/{name}/proposals/{id}", h.handleProposalDetail)
	mux.HandleFunc("GET /r/{owner}/{name}/releases", h.handleReleases)
	mux.HandleFunc("GET /r/{owner}/{name}/releases/{rname}", h.handleReleaseDetail)
	mux.HandleFunc("GET /r/{owner}/{name}/ci-jobs", h.handleCIJobs)
	mux.HandleFunc("GET /r/{owner}/{name}/ci-jobs/{id}", h.handleCIJobDetail)
	mux.HandleFunc("GET /o/{org}/invites/{token}/accept", h.handleAcceptInvite)
	mux.HandleFunc("POST /o/{org}/invites/{token}/accept", h.handleAcceptInvite)
	mux.HandleFunc("GET /orgs/new", h.handleCreateOrg)
	mux.HandleFunc("POST /orgs/new", h.handleCreateOrg)
	mux.HandleFunc("GET /repos/new", h.handleCreateRepo)
	mux.HandleFunc("POST /repos/new", h.handleCreateRepo)

	// Branch write operations (POST-redirect-GET pattern).
	mux.HandleFunc("POST /r/{owner}/{name}/-/create-branch", h.handleUICreateBranch)
	mux.HandleFunc("POST /r/{owner}/{name}/-/delete-branch", h.handleUIDeleteBranch)
	mux.HandleFunc("POST /r/{owner}/{name}/-/promote-branch", h.handleUIPromoteBranch)
	mux.HandleFunc("POST /r/{owner}/{name}/-/set-auto-merge", h.handleUISetAutoMerge)

	// Write operations — POST forms from branch detail and proposals pages.
	mux.HandleFunc("POST /r/{owner}/{name}/b/{branch...}", h.handleBranchPostDispatch)
	mux.HandleFunc("POST /r/{owner}/{name}/proposals", h.handleCreateProposalUI)
	mux.HandleFunc("POST /r/{owner}/{name}/proposals/{id}/edit", h.handleEditProposal)
	mux.HandleFunc("POST /r/{owner}/{name}/proposals/{id}/close", h.handleCloseProposalUI)
	// Write operations: org members.
	mux.HandleFunc("POST /o/{org}/members", h.handleAddOrgMember)
	mux.HandleFunc("POST /o/{org}/members/{identity}/remove", h.handleRemoveOrgMember)
	// Write operations: org invites.
	mux.HandleFunc("POST /o/{org}/invites", h.handleCreateInvite)
	mux.HandleFunc("POST /o/{org}/invites/{id}/revoke", h.handleRevokeInvite)
	// Write operations: repo roles.
	mux.HandleFunc("POST /r/{owner}/{name}/roles", h.handleSetRole)
	mux.HandleFunc("POST /r/{owner}/{name}/roles/{identity}/delete", h.handleDeleteRole)
	// Write operations: releases.
	mux.HandleFunc("POST /r/{owner}/{name}/releases", h.handleCreateRelease)
	mux.HandleFunc("POST /r/{owner}/{name}/releases/{rname}/delete", h.handleDeleteRelease)
	// Write operations: delete org/repo.
	mux.HandleFunc("POST /o/{org}/delete", h.handleUIDeleteOrg)
	mux.HandleFunc("POST /r/{owner}/{name}/-/delete-repo", h.handleUIDeleteRepo)
	// (commit form is handled by handleBranchGetDispatch / handleBranchPostDispatch above)

	mux.Handle("GET /static/", http.StripPrefix("/static/", http.FileServer(http.FS(h.staticSub))))
}
