...
// WriteStoreLite is the subset of server.WriteStore that the UI needs.
...
// IdentityFn extracts the caller's authenticated identity from a request context.
// It is injected by the server so the UI does not need to import the server package.
